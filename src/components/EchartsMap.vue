...
const setOptions = () => {
	...
	myCharts?.setOption(options) //?：myCharts有值时执行
}
const getLineData = () => {
	/**
	 * [
	 * {coords：[[第一网格]，[第二网格]，gridLevel:第一网格.gridLevel},
	 * {coords：[[第一网格]，[第三网格]，gridLevel:第一网格.gridLevel},
	 * {coords：[[第一网格]，[第四网格]，gridLevel:第一网格.gridLevel},
	 * {coords：[[第二网格]，[第一网格]，gridLevel:第一网格.gridLevel},
	 * ...十二条
	 * ]
	 */
	const result = []
...
#container {
	width: 100%; //拿到整个可视区域宽度
	height: 80%;
	position: absolute;
	z-index: 1;
	//pointer-events: none; //关闭地图所有事件（点击事件...）
}
</style>
